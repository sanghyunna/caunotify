...
async function crawlGerman(url,n){
    url = "http://germania.cau.ac.kr/cm/cm_1.php";
    if(n == 2){
        url = "http://germania.cau.ac.kr/cm/cm_1.php?idx=&p_idx=&s_kind=&s_scroll=all&s_key=&p_page=2&code=b_1&p_listtype=&p_mode=list&mode=&p_pgfile=%2Fcm%2Fcm_1.php";
    }
    let url_list = [];
    let title_list = [];
    ...
    //    fs.appendFileSync('pup.json', `${data}\n`);
        if(element.attribs.href != undefined){
            if(element.attribs.href.startsWith(`javascript:view('`)){ // 17 20
                ...
                url_list.push(`http://germania.cau.ac.kr/cm/cm_1.php?idx=&p_idx=${trimmedLink}&s_kind=&s_scroll=all&s_key=&p_page=1&code=b_1&p_listtype=&p_mode=view&mode=&p_pgfile=%2Fcm%2Fcm_1.php`);
